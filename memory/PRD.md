...
- **Point 3 (cul-de-sac fin de pile)** — Cause : ancien code n'affichait qu'un `.b1-opp-empty` avec 2 lignes de texte. **Nouveau composant `B1FinDePile.jsx`** :
  - **Sablier SVG animé** (2 animations : opacité en haut, hauteur du sable en chute au centre, dépôt en bas)
  - Texte : "*De nouvelles opportunités de mandat vous attendent dans*"
  - **Décompte HH:MM:SS jusqu'à 03h00 Europe/Paris** — calcul DST-safe via `Intl.DateTimeFormat({timeZone: 'Europe/Paris'})`, met à jour toutes les secondes
  - Label "HEURES — MINUTES — SECONDES"
  - **Slot veille** : si `/api/me/veille` retourne actif + cartes, `VeilleIntercalaire` s'affiche EN DESSOUS du décompte (pas en remplacement). Screenshot preview confirme : sablier + `02:20:04` visible sur zone calme.
- **Point 4 (logo carré vide)** — Cause racine trouvée : le PNG fourni (`kolo-mark-black-256.png`, 2132 bytes) a un **centre transparent** (pixel(128,128) = (0,0,0,0)) et des edges dark navy sur x=64/x=192. Sur le fond violet #F0EEF8, le centre transparent laisse voir le violet → aspect "cadre rectangulaire avec trou" perçu comme "carré vide". **Fix** : remplacement par un **SVG texte inline `KOLO` en League Spartan noir gras**. Aucun risque de chemin cassé, CSP, ou aspect déformé au packaging Capacitor. Rendu preview : "KOLO" gras noir bien lisible.
- **Point 5 (assistant)** — `SYSTEM_PROMPT` durci : "*Vos réponses font trois à quatre phrases courtes maximum, suivies du prochain pas concret. Vous n'énumérez jamais plus de trois points. Vous ne faites jamais de titres ni de sections.*" `max_tokens: 300 → 400`. **Streaming SSE en WebView Capacitor iOS** — connu pour buffer les chunks. Implémentation actuelle : LLM répond en entier côté serveur, puis serveur envoie chunks de 24 chars via SSE. Sur iOS WebView Capacitor, tout arrive d'un coup → percu comme "lent puis instantané". Solution complète (streaming natif ChatCompletion via emergentintegrations) demanderait plus de temps — priorité au max_tokens qui raccourcit drastiquement l'attente.
- **Point 6 (prod DupliCateKey)** — Le fix est en place depuis la 2e passe : `a2/indexes.py` crée l'index avec `partialFilterExpression`, ET `_auto_migrer_prod` tourne à chaque startup FastAPI (idempotent). Vérifiable dans les logs supervisord : `[auto-migrer-prod] N index recréé(s)`. Après le prochain déploiement backend, la prod se répare toute seule au boot.
- **Build stamp** — inchangé, `B1BuildStamp` monté depuis 2 passes. Sur le préview capture : `build dev-75190148` visible bas login.
- **12 tests de régression** dans `/app/backend/tests/test_testflight_v3.py` : endpoint `/swipe` avec statuts corrects, pas de redirect Estimation, erreur visible, `FinDePileScreen` monté avec sablier + décompte, tab bar opaque avec bord rose franc, logo SVG texte (pas PNG à trou), prompt 3-4 phrases, max_tokens 400, auto-migrer-prod au startup. **43/43 tests critiques verts** (28 antérieurs + 12 v3 + 3 déjà comptés).

### BLOC D · Fixes TestFlight 2e passe (Sep 3, 2026)
