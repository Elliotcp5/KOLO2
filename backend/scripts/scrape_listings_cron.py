...
MAX_ZIPS_PER_BATCH = 20        # actor memory cap → 20 ZIPs / run
MAX_ITEMS_PER_ZIP = 30          # 30 listings × 20 ZIPs = 600 items / run
POLL_INTERVAL_SEC = 5
POLL_MAX_SEC = 240              # 4 min per batch max
...
async def _scrape_batch(client: httpx.AsyncClient, zips: list[str]) -> dict:
    """Run one Apify batch → dedupe → upsert to Supabase.
    Returns { zips, upserted, kept_after_dedupe, status }.
    """
    logger.info(f"Kicking off Apify for ZIPs: {zips}")
    run_id, dataset_id = await _kickoff_apify(client, zips)
    if not run_id or not dataset_id:
        return {"zips": zips, "upserted": 0, "kept_after_dedupe": 0, "status": "kickoff_failed"}

    rows = await _poll_and_fetch(client, run_id, dataset_id)
    if rows is None:
        return {"zips": zips, "upserted": 0, "kept_after_dedupe": 0, "status": "no_data"}

    unique = _dedupe_by_url(rows)
    logger.info(f"  → {len(rows)} raw items, {len(unique)} unique URLs")

    # Import the upsert helper lazily to avoid heavy server.py import chain at
    # top of the file (server initialises Mongo/etc). We only need this fn.
...
async def run_once(explicit_zips: list[str] | None = None) -> dict:
    ...
    try:
        ...
        async with httpx.AsyncClient() as http_client:
            for batch in _batch(target_zips, MAX_ZIPS_PER_BATCH):
                res = await _scrape_batch(http_client, batch)
                results.append(res)
                total_upserted += res.get("upserted", 0) or 0
                total_unique += res.get("kept_after_dedupe", 0) or 0
                # Small breather between batches to be nice to Apify's queue.
